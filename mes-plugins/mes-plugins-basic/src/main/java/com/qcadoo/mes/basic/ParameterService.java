...
public class ParameterService {
    ...
    public Entity getParameter() {
        ...
        if (parameterDD != null) {
            //standlone parameter = parameterDD.find("SELECT p FROM #basic_parameter p").setMaxResults(1).setCacheable(true).uniqueResult();
//Alex_Jiang 单机版本需要去掉  ricefishM
            SearchQueryBuilder searchQueryBuilder=parameterDD.find("SELECT p FROM #basic_parameter p WHERE p.tenantId = :tenant_id");
            searchQueryBuilder.setLong("tenant_id", multiTenantService.getCurrentTenantId());
            parameter = searchQueryBuilder.setMaxResults(1).setCacheable(true).uniqueResult();
            if (parameter == null) {
