...
class BigDecimalCellParser implements CellParser {
    ...
    public static final String ERROR_CODE_INVALID_NUMERIC_FORMAT = "smartView.validate.field.error.invalidNumericFormat";

    private static final Pattern CHINESE_DECIMAL_PATTERN = Pattern.compile("^-?\\d+(,\\d+)?$");

    private static final Pattern ENGLISH_AND_GERMAN_DECIMAL_PATTERN = Pattern.compile("^-?\\d+(\\.\\d+)?$");

    private static final Locale CHINESE_LOCALE = new Locale("zh");
    ...
    private boolean validateDecimalFormat(String decimalString, Locale locale, BindingErrorsAccessor errorsAccessor) {
        ...
        if (CHINESE_LOCALE.getLanguage().equals(language)) {
            decimalPattern = CHINESE_DECIMAL_PATTERN;
