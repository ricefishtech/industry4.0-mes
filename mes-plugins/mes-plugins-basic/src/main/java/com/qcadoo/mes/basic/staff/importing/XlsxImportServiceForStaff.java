...
@Service
public class XlsxImportServiceForStaff {

    private final RowProcessorFactoryForStaff rowProcessorFactoryForStaff;

    @Autowired
    public XlsxImportServiceForStaff(RowProcessorFactoryForStaff rowProcessorFactoryForStaff) {
        this.rowProcessorFactoryForStaff = rowProcessorFactoryForStaff;
    }
    ...
    public ImportStatus importFrom(final XSSFWorkbook workbook) {
        ...
        for (int rowIndex = SpreadsheetSchemaInfo.START_ROW_INDEX; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
            ...
            final RowProcessor rowProcessor = rowProcessorFactoryForStaff.create(importStatus, rowIndex);
