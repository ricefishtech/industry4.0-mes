...
public abstract class AbstractSamplesLoader implements SamplesLoader {
     ...
     * 
     * @param locale
     *            language code (ISO-639 code). Currently supported language codes are "zh" "pl" and "en".
     */
    ...
    protected void addDictionaryItems(final Map<String, String> values) {
        ...
        if (dictionary != null) {
            if (!checkIfDictionaryItemExists(values.get("item"))) {
                Entity item = dataDefinitionService.get("smartModel", "dictionaryItem").create();
    ...
    protected String getRandomDictionaryItem(final String dictionaryName) {
        ...
        SearchCriteriaBuilder searchBuilder = dataDefinitionService.get("smartModel", "dictionaryItem").find()
    ...
    protected Entity getDictionaryByName(final String name) {
        return dataDefinitionService.get("smartModel", "dictionary").find().add(SearchRestrictions.eq(L_NAME, name))
...
//ricefishM
        parameter.setField("priceBasedOn", "02realProductionCost");
        parameter.setField("warehouseIssueProductsSource","02manual");
        parameter.setField("drawnDocuments","01transfer");
        parameter.setField("documentState","02accepted");
        parameter.setField("documentsStatus","01accepted");
        parameter.setField("productsToIssue","01allInputProducts");
        parameter.setField("maxProductsQuantity","500");
    ...
    private Entity getDictionaryItem(final String itemName) {
        return dataDefinitionService.get("smartModel", "dictionaryItem").find().add(SearchRestrictions.eq(L_NAME, itemName))
                .setMaxResults(1).uniqueResult();
    }
