...
public class PerformanceAnalysisListeners {
    ...
    public void showDetails(final ViewDefinitionState view, final ComponentState state, final String[] args) {
        ...
        List<Long> selectedEntities = Lists.newArrayList(performanceAnalysisGrid.getSelectedEntitiesIds());

        Entity analysis = dataDefinitionService
                .get(ProductionCountingConstants.PLUGIN_IDENTIFIER, PerformanceAnalysisDtoFields.MODEL_PERFORMANCE_ANALYSIS_DTO)
                .get(selectedEntities.get(0));
        if (analysis != null) {
    ...
    public void calculateTotalTime(final ViewDefinitionState view, final ComponentState state, final String[] args) {
        ...
        if (StringUtils.isNoneBlank(filterQ)) {
            query = query + " WHERE " + filterQ + " AND tenantid="+multiTenantService.getCurrentTenantId();//ricefishM
        }
