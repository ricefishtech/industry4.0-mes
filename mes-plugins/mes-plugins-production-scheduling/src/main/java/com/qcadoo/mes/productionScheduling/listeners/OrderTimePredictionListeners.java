    ...
    public void changeRealizationTime(final ViewDefinitionState view, final ComponentState state, final String[] args) {
        ...
        if (value instanceof BigDecimal) {
            ...
            try {
                ...
                plannedQuantityField.addMessage("smartView.validate.field.error.invalidNumericFormat", MessageType.FAILURE);
        ...
        if (MAX != null && scale > MAX) {
            plannedQuantityField.addMessage("smartView.validate.field.error.invalidScale.max", MessageType.FAILURE,
                    MAX.toString());
            return;
        }
        ...
        if (MAX != null && presicion > MAX) {
            plannedQuantityField.addMessage("smartView.validate.field.error.invalidPrecision.max", MessageType.FAILURE,
                    MAX.toString());
            return;
        }

        if (BigDecimal.ZERO.compareTo(quantity) >= 0) {
            plannedQuantityField.addMessage("smartView.validate.field.error.outOfRange.toSmall", MessageType.FAILURE);
            return;
        }
