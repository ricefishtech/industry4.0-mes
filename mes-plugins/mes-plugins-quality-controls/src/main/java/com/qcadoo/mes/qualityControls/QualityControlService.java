...
public final class QualityControlService {
    ...
    private static final String QCADOO_VIEW_VALIDATE_GLOBAL_ERROR_CUSTOM = "smartView.validate.global.error.custom";
    ...
    public void closeQualityControl(final ViewDefinitionState viewDefinitionState, final ComponentState state, final String[] args) {
        if (state.getFieldValue() == null) {
            if (state instanceof FormComponent) {
                state.addMessage("smartView.form.entityWithoutIdentifier", MessageType.FAILURE);
            } else {
                state.addMessage("smartView.grid.noRowSelectedError", MessageType.FAILURE);
            }
